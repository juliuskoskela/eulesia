          ...
          <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
            <table className="w-full">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">{t('users.name')}</th>
                  ...
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">{t('users.joined')}</th>
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">{t('users.lastSeen')}</th>
                </tr>
              </thead>
                ...
                {data?.items?.map(user => (
                  <tr key={user.id} className="hover:bg-gray-50">
                    <td className="px-4 py-3">
                      <Link to={`/admin/users/${user.id}`} className="flex items-center gap-3 hover:text-blue-600">
                    ...
                    <td className="px-4 py-3 text-sm text-gray-500">{formatRelativeTime(user.createdAt)}</td>
          ...
          {data && data.total > 20 && (
            <div className="flex items-center justify-between mt-4">
              <span className="text-sm text-gray-500">
                {t('users.showing', { from: (page - 1) * 20 + 1, to: Math.min(page * 20, data.total), total: data.total })}
              </span>
...
function RoleBadge({ role }: { role: string }) {
  ...
  return (
    <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${colors[role] || 'bg-gray-100 text-gray-800'}`}>
      {role}
    </span>
  )
}
