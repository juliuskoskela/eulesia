...
const REASONS = ['illegal', 'harassment', 'spam', 'misinformation', 'other'] as const

export function ReportDialog({ isOpen, onClose, contentType, contentId }: ReportDialogProps) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" role="dialog" aria-modal="true" aria-labelledby="report-dialog-title" ref={dialogRef}>
      ...
      <div className="relative bg-white dark:bg-gray-900 rounded-xl shadow-xl w-full max-w-md p-6 z-10">
        <div className="flex items-center justify-between mb-4">
          <h2 id="report-dialog-title" className="text-lg font-semibold text-gray-900 dark:text-gray-100">{t('report.title')}</h2>
        ...
        <div className="space-y-4">
          ...
          <div className="flex gap-2 justify-end">
            ...
            >
              {submitReportMutation.isPending && <Loader2 className="w-4 h-4 animate-spin" />}
              {t('report.submit')}
            </button>
          </div>
