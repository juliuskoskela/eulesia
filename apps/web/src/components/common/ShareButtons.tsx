...
interface ShareButtonsProps {
  url: string;
  title: string;
  /** Compact mode: just a share icon button that opens popup */
  compact?: boolean;
}
...
export function ShareButtons({
            ...
            <div className="absolute right-0 top-full mt-1 z-20 bg-white dark:bg-gray-900 rounded-lg shadow-lg border border-gray-200 dark:border-gray-800 p-3 w-56">
              ...
              >
                {copied ? (
                  <>
                    ...
                    <span className="text-green-600">{t("share.copied")}</span>
                  </>
                ) : (
                  <>
              ...
              <div className="flex gap-1.5 mt-2">
                {SHARE_TARGETS.map((target) => (
                  <a
                    key={target.id}
                    href={target.getUrl(fullUrl, title)}
                    target="_blank"
                    rel="noopener noreferrer"
                    className={`flex-1 flex items-center justify-center p-2 rounded-lg text-white transition-colors ${target.color}`}
                    title={target.label}
                  >
                    <target.icon />
                  </a>
  ...
  // Inline mode: horizontal share bar
  return (
    <div className="flex items-center gap-2">
      ...
      {SHARE_TARGETS.map((target) => (
        <a
          ...
          target="_blank"
          rel="noopener noreferrer"
          className={`inline-flex items-center justify-center w-8 h-8 rounded-lg text-white transition-colors ${target.color}`}
          title={target.label}
        >
          <target.icon />
        </a>
