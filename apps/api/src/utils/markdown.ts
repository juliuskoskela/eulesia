...
const IMAGE_URL_REGEX =
  /^(https?:\/\/[^\s]+\.(?:jpg|jpeg|png|gif|webp|avif|bmp)(?:\?[^\s]*)?)\n?/i;

const imageEmbedExtension: TokenizerExtension & RendererExtension = {
  name: "imageEmbed",
  level: "block",
  start(src: string) {
    return src.match(/^https?:\/\/[^\s]+\.(?:jpg|jpeg|png|gif|webp|avif|bmp)/i)
      ?.index;
  },
  tokenizer(src: string) {
...
marked.use({
  extensions: [youtubeExtension, imageEmbedExtension, linkPreviewExtension],
});
