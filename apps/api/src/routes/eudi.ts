...
router.post(
  ...
  asyncHandler(async (_req: Request, res: Response) => {
    const { sessionId, requestUri, presentationRequest } =
      await eudiService.createAuthSession();
...
router.post(
  "/callback",
  asyncHandler(async (req: Request, res: Response) => {
    const { vp_token, presentation_submission, state } = callbackSchema.parse(
      req.body,
    );

    const session = await eudiService.processCallback(
      state,
      vp_token,
