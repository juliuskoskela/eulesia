...
router.get('/dashboard', asyncHandler(async (_req: AuthenticatedRequest, res: Response) => {
  ...
  const [pendingReports] = await db.select({ count: count() }).from(contentReports)
    .where(eq(contentReports.status, 'pending'))
  const [pendingAppeals] = await db.select({ count: count() }).from(moderationAppeals)
...
router.get('/users', asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  ...
  if (role && ['citizen', 'institution', 'admin'].includes(role as string)) {
    conditions.push(eq(users.role, role as 'citizen' | 'institution' | 'admin'))
  }
...
router.get('/users/:id', asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  ...
  const [threadCount] = await db.select({ count: count() }).from(threads).where(eq(threads.authorId, id))
  const [commentCount] = await db.select({ count: count() }).from(comments).where(eq(comments.authorId, id))
...
router.patch('/users/:id/role', asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  const { id } = req.params
...
router.post('/users/:id/sanction', asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  const { id } = req.params
  const { sanctionType, reason, expiresAt } = issueSanctionSchema.parse(req.body)

  const [user] = await db.select().from(users).where(eq(users.id, id)).limit(1)
  if (!user) throw new AppError(404, 'User not found')

  if (user.role === 'admin') throw new AppError(400, 'Cannot sanction an admin')

  const [sanction] = await db.insert(userSanctions).values({
    userId: id,
    sanctionType,
    reason,
  ...
  // Log action
  const actionType = sanctionType === 'warning' ? 'user_warned'
  ...
  await db.insert(notifications).values({
    ...
    title: sanctionType === 'warning' ? 'You have received a warning'
      : sanctionType === 'suspension' ? 'Your account has been suspended'
...
router.get('/users/:id/sanctions', asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  const { id } = req.params
  const sanctions = await db.select({
    id: userSanctions.id,
...
router.delete('/sanctions/:id', asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  const { id } = req.params

  const [sanction] = await db.select().from(userSanctions).where(eq(userSanctions.id, id)).limit(1)
  if (!sanction) throw new AppError(404, 'Sanction not found')

  await db.update(userSanctions).set({
    revokedAt: new Date(),
...
router.get('/reports', asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  ...
  const conditions = []
  if (status && ['pending', 'reviewing', 'resolved', 'dismissed'].includes(status as string)) {
    conditions.push(eq(contentReports.status, status as any))
  }
  if (reason && ['illegal', 'harassment', 'spam', 'misinformation', 'other'].includes(reason as string)) {
...
router.get('/reports/:id', asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  const { id } = req.params
  ...
  switch (report.contentType) {
    case 'thread': {
      const [t] = await db.select().from(threads).where(eq(threads.id, report.contentId)).limit(1)
      content = t
      break
    }
    case 'comment': {
      const [c] = await db.select().from(comments).where(eq(comments.id, report.contentId)).limit(1)
      content = c
      break
    }
    case 'club_thread': {
      const [ct] = await db.select().from(clubThreads).where(eq(clubThreads.id, report.contentId)).limit(1)
      content = ct
      break
    }
    case 'club_comment': {
      const [cc] = await db.select().from(clubComments).where(eq(clubComments.id, report.contentId)).limit(1)
      content = cc
      break
    }
    case 'club': {
      const [cl] = await db.select().from(clubs).where(eq(clubs.id, report.contentId)).limit(1)
      content = cl
      break
    }
    case 'user': {
      ...
      }).from(users).where(eq(users.id, report.contentId)).limit(1)
      content = u
      break
    }
  }
...
router.patch('/reports/:id', asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  ...
  const [report] = await db.select().from(contentReports).where(eq(contentReports.id, id)).limit(1)
  if (!report) throw new AppError(404, 'Report not found')
  ...
  await db.update(contentReports).set(updates).where(eq(contentReports.id, id))

  // Log action
  const actionType = status === 'dismissed' ? 'report_dismissed' : 'report_resolved'
  if (status === 'resolved' || status === 'dismissed') {
    await db.insert(moderationActions).values({
      adminUserId: req.user!.id,
      actionType,
...
router.delete('/content/:type/:id', asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  const { type, id } = req.params
  const { reason } = req.body || {}

  switch (type) {
    case 'thread':
      await db.update(threads).set({ isHidden: true } as any).where(eq(threads.id, id))
      break
    case 'comment':
      await db.update(comments).set({ isHidden: true } as any).where(eq(comments.id, id))
      break
    case 'club_thread':
      await db.update(clubThreads).set({ isHidden: true } as any).where(eq(clubThreads.id, id))
      break
    case 'club_comment':
      await db.update(clubComments).set({ isHidden: true } as any).where(eq(clubComments.id, id))
      break
    default:
...
router.post('/content/:type/:id/restore', asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  const { type, id } = req.params

  switch (type) {
    case 'thread':
      await db.update(threads).set({ isHidden: false } as any).where(eq(threads.id, id))
      break
    case 'comment':
      await db.update(comments).set({ isHidden: false } as any).where(eq(comments.id, id))
      break
    case 'club_thread':
      await db.update(clubThreads).set({ isHidden: false } as any).where(eq(clubThreads.id, id))
      break
    case 'club_comment':
      await db.update(clubComments).set({ isHidden: false } as any).where(eq(clubComments.id, id))
      break
    default:
      throw new AppError(400, 'Invalid content type')
  }
...
router.get('/modlog', asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  ...
  const [totalResult] = await db.select({ count: count() }).from(moderationActions).where(whereClause)
...
router.get('/transparency', asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  ...
  // Reports by reason
  const reportsByReason = await db.select({
    reason: contentReports.reason,
    count: count()
  })
    .from(contentReports)
  ...
  const reportsByContentType = await db.select({
    contentType: contentReports.contentType,
    count: count()
  })
    .from(contentReports)
  ...
  const sanctionsByType = await db.select({
    sanctionType: userSanctions.sanctionType,
    count: count()
  })
    .from(userSanctions)
  ...
  const appealsByStatus = await db.select({
  ...
  const [avgResponseTime] = await db.select({
    ...
    .where(and(
      dateCondition,
      sql`${contentReports.resolvedAt} IS NOT NULL`
    ))
  ...
  res.json({
    ...
    data: {
      ...
      reports: {
        ...
        avgResponseTimeHours: avgResponseTime?.avg ? Math.round(avgResponseTime.avg * 10) / 10 : null
      },
...
router.get('/appeals', asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  ...
  const whereClause = conditions.length > 0 ? and(...conditions) : undefined
  const [totalResult] = await db.select({ count: count() }).from(moderationAppeals).where(whereClause)
...
router.patch('/appeals/:id', asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  const { id } = req.params
  const { status, adminResponse } = resolveAppealSchema.parse(req.body)

  const [appeal] = await db.select().from(moderationAppeals).where(eq(moderationAppeals.id, id)).limit(1)
  if (!appeal) throw new AppError(404, 'Appeal not found')

  await db.update(moderationAppeals).set({
    status,
    adminResponse,
