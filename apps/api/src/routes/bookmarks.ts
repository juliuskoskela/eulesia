...
import { db, bookmarks, threads, users, municipalities, threadTags, threadVotes } from '../db/index.js'
...
router.delete('/:threadId', authMiddleware, asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  const userId = req.user!.id
...
router.get('/', authMiddleware, asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  ...
  const userVotes = votes.reduce((acc, v) => {
