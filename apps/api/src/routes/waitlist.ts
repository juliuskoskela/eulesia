...
router.post('/join', asyncHandler(async (req: Request, res: Response) => {
  ...
  const genericMsg = 'If this email is not already registered, you have been added to the waitlist.'
...
router.get('/admin/stats', authMiddleware, requireAdmin, asyncHandler(async (_req: AuthenticatedRequest, res: Response) => {
  const [pending] = await db.select({ count: count() }).from(waitlist).where(eq(waitlist.status, 'pending'))
  const [approved] = await db.select({ count: count() }).from(waitlist).where(eq(waitlist.status, 'approved'))
  const [rejected] = await db.select({ count: count() }).from(waitlist).where(eq(waitlist.status, 'rejected'))
...
router.get('/admin', authMiddleware, requireAdmin, asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  ...
  if (status && ['pending', 'approved', 'rejected'].includes(status as string)) {
    conditions.push(eq(waitlist.status, status as 'pending' | 'approved' | 'rejected'))
  }
  const whereClause = conditions.length > 0 ? and(...conditions) : undefined

  const [totalResult] = await db.select({ count: count() }).from(waitlist).where(whereClause)
...
router.post('/admin/:id/approve', authMiddleware, requireAdmin, asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  const { id } = req.params
  ...
  if (entry.status !== 'pending') {
    res.status(400).json({ success: false, error: 'Entry already processed' })
    return
  }
  ...
  await db.transaction(async (tx) => {
    const [newCode] = await tx.insert(inviteCodes).values({
      code,
      createdBy: adminId,
      status: 'available',
  ...
  const emailSent = await emailService.sendWaitlistApproval(entry.email, code, entry.locale || 'en')

  if (emailSent) {
    await db.update(waitlist).set({ emailSentAt: new Date() }).where(eq(waitlist.id, id))
  }

  res.json({ success: true, data: { id, status: 'approved', code, emailSent } })
}))

// POST /waitlist/admin/:id/reject — Reject entry
router.post('/admin/:id/reject', authMiddleware, requireAdmin, asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  const { id } = req.params
...
router.post('/admin/bulk-approve', authMiddleware, requireAdmin, asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  ...
  for (const id of ids) {
    const [entry] = await db.select().from(waitlist).where(and(eq(waitlist.id, id), eq(waitlist.status, 'pending'))).limit(1)
    if (!entry) continue
