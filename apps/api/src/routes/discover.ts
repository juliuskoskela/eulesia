...
import { db, trendingCache, threads, users, municipalities, threadTags, threadVotes } from '../db/index.js'
import { optionalAuthMiddleware } from '../middleware/auth.js'
import { asyncHandler } from '../utils/asyncHandler.js'
import { getExploreFeed, getAlgorithmDocumentation } from '../services/trending.js'
...
router.get('/explore', optionalAuthMiddleware, asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  ...
  const tagResults = threadIds.length > 0
    ? await db.select().from(threadTags).where(inArray(threadTags.threadId, threadIds))
    : []

  const tagsByThread = tagResults.reduce((acc, tag) => {
  ...
  if (sourceInstIds.length > 0) {
    ...
    srcInstNames = insts.reduce((acc, i) => {
