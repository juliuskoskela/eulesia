...
export function computeCivicValueScore(thread: {
  ...
  const halflife =
    thread.source === "minutes_import" || thread.source === "rss_import"
      ? 72
      : 24;
...
export async function getExploreFeed(
  ...
  if (options.scope && options.scope !== "all") {
    conditions.push(
      eq(threads.scope, options.scope as "local" | "national" | "european"),
    );
  }
  ...
  const replierMap = new Map(
    replierCounts.map((r) => [r.threadId, Number(r.uniqueRepliers)]),
  );
...
> {
  ...
  const candidates = await db
    ...
    .from(threads)
    .where(and(eq(threads.isHidden, false), gt(threads.createdAt, twoDaysAgo)));
  ...
  for (const thread of candidates) {
    ...
    const totalInteractions =
      Math.abs(thread.score || 0) + (thread.replyCount || 0);
    const ageHours =
      (Date.now() - (thread.createdAt?.getTime() || Date.now())) /
      (1000 * 60 * 60);
...
> {
  ...
  const recentTags = await db
    ...
    .where(and(eq(threads.isHidden, false), gt(threads.createdAt, oneDayAgo)))
  ...
  const weeklyTags = await db
    ...
    .from(threadTags)
    .innerJoin(threads, eq(threadTags.threadId, threads.id))
    .where(and(eq(threads.isHidden, false), gt(threads.createdAt, oneWeekAgo)))
    .groupBy(threadTags.tag);
...
export async function refreshTrendingCache(): Promise<void> {
  ...
  try {
    ...
    console.log(
      `   ✅ Trending cache refreshed: ${trendingThreads.length} threads, ${trendingTagsData.length} tags`,
    );
  } catch (err) {
    console.error(
...
export async function batchUpdateViewCounts(): Promise<void> {
  try {
    ...
    console.error(
      "View count batch update failed:",
      err instanceof Error ? err.message : err,
    );
  }
}
