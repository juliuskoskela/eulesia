...
export async function searchLocations(
...
): Promise<{ results: LocationResult[]; source: 'cache' | 'nominatim' | 'mixed' }> {
  const limit = options.limit || 10
  const country = options.country?.toUpperCase() || 'FI'
  const includeNominatim = options.includeNominatim !== false

  // 1. Search local database
  const localResults = await searchLocalDatabase(query, { ...options, limit, country })
  ...
  return {
    ...
    source: localResults.length > 0 && nominatimResults.length > 0
      ? 'mixed'
...
async function searchLocalDatabase(
  ...
  return results.map(loc => ({
    ...
    bounds: loc.bounds as { south: number; north: number; west: number; east: number } | null,
    population: loc.population || null,
    status: 'active' as const,
    contentCount: loc.contentCount || 0,
    parent: loc.parentId && parents.get(loc.parentId)
      ? { name: parents.get(loc.parentId)!.name, type: parents.get(loc.parentId)!.type || 'region' }
      : null
  }))
}
...
async function searchNominatimLocations(
  ...
  try {
    ...
    filtered = filtered.filter(r =>
      r.category === 'boundary' ||
      r.category === 'place' ||
      ['city', 'town', 'village', 'municipality', 'county', 'state', 'region'].includes(r.type)
    )
 ...
 */
export async function findLocationByOsmId(osmId: number): Promise<Location | null> {
 ...
 */
async function createLocationFromNominatim(result: NominatimResult): Promise<Location> {
  const names = extractLocalizedNames(result)
  const bounds = extractBounds(result)
  const population = extractPopulation(result)
  const type = mapNominatimToLocationType(result)
  ...
  if (parentInfo && result.address) {
    ...
    const [existingParent] = await db
      ...
      .where(and(
        ...
        eq(locations.country, result.address.country_code?.toUpperCase() || 'FI')
      ))
      .limit(1)
 ...
 */
export async function getLocationHierarchy(locationId: string): Promise<LocationHierarchy[]> {
...
export async function lookupLocation(
  ...
  if (existing) {
    ...
    return {
      ...
      bounds: existing.bounds as { south: number; north: number; west: number; east: number } | null,
