...
async function isAlreadyImported(sourceId: string): Promise<boolean> {
  const existing = await db
    ...
    .from(threads)
    .where(
      and(eq(threads.sourceId, sourceId), eq(threads.source, "rss_import")),
    )
    .limit(1);
...
export async function importEuContent(
  ...
  const { sources: filterSources, dryRun = false, limit = 10 } = options;
  ...
  if (filterSources?.length) {
    ...
    sources = sources.filter((s) =>
      filterLower.includes(s.institution.toLowerCase()),
    );
  }
      ...
      for (const item of feed.items) {
        ...
        try {
          ...
          if (item.link && item.link.includes("presscorner")) {
            const apiContent = await fetchPresscornerContent(
              item.link,
              item.id,
            );
            if (apiContent) {
              ...
              console.log(
                `   ✅ Using presscorner API content (${fullContent.length} chars)`,
              );
            }
          }
          ...
          if (fullContent.length < 500) {
            ...
            if (item.categories?.length)
              enriched.push(`Categories: ${item.categories.join(", ")}`);
            enriched.push(`\n${fullContent}`);
            fullContent = enriched.join("\n");
            console.log(
              `   ℹ️  Using enriched feed description (${fullContent.length} chars)`,
            );
          }
          ...
          if (isLowQualityContent(fullContent)) {
            console.log(
              `   ⏭️  Skipping low-quality content: ${item.title.slice(0, 50)}`,
            );
            result.skipped++;
            continue;
          }
          ...
          const allTags = [
            ...
            "eu",
            source.contentType,
            source.institution.toLowerCase().split(" ").pop()!,
          ];
 ...
 */
async function fetchPresscornerContent(
  link: string,
  guid: string,
): Promise<string | null> {
  ...
  try {
    ...
    if (!response.ok) {
      console.log(
        `   ⚠️  Presscorner API returned ${response.status} for ${apiRef}`,
      );
      return null;
    }
    ...
    if (!resource?.htmlContent) {
      console.log(
        `   ⚠️  No htmlContent in presscorner API response for ${apiRef}`,
      );
      return null;
    }
    ...
    if (plainText.length < 100) {
      console.log(
        `   ⚠️  Presscorner content too short (${plainText.length} chars) for ${apiRef}`,
      );
      return null;
    }

    console.log(
      `   ✅ Got ${plainText.length} chars from presscorner API for ${apiRef}`,
    );
    return plainText.slice(0, 30000);
  } catch (err) {
    console.log(
      `   ⚠️  Presscorner API error for ${apiRef}: ${err instanceof Error ? err.message : err}`,
    );
    return null;
  }
}
...
function isLowQualityContent(text: string): boolean {
  ...
  const matchCount = garbageIndicators.filter((pattern) =>
    pattern.test(text),
  ).length;
