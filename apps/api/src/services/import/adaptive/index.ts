...
function parseDate(dateStr: string, format: string): string | undefined {
  ...
  switch (format) {
    ...
    case 'DD/MM/YYYY': {
      ...
      const match = cleaned.match(new RegExp(`(\\d{1,2})${sep}(\\d{1,2})${sep}(\\d{4})`))
...
const configCache = new Map<string, { config: FetcherConfig; fetcherOptions: Record<string, string> | null; expiry: number }>()
const CACHE_TTL_MS = 5 * 60 * 1000 // 5 minutes

async function getConfig(configId: string): Promise<{ config: FetcherConfig; fetcherOptions: Record<string, string> | null } | null> {
  ...
  const rows = await scraperDb
    .select({ config: scraperConfigs.config, fetcherOptions: scraperConfigs.fetcherOptions })
    .from(scraperConfigs)
...
export const adaptiveFetcher: MinuteFetcher = {
  ...
  async fetchMeetings(source: MinuteSource): Promise<Meeting[]> {
    ...
    if (!response.ok) {
      throw new Error(`Adaptive fetch failed: ${response.status} for ${listUrl}`)
    }
    ...
    while ((match = regex.exec(html)) !== null) {
      ...
      const title = groups.title !== undefined ? match[groups.title]?.trim() : undefined
      const dateRaw = groups.date !== undefined ? match[groups.date] : undefined
      const organ = groups.organ !== undefined ? match[groups.organ]?.trim() : undefined
...
async function extractPdf(
  ...
  try {
    ...
    console.log(`   [adaptive] PDF extraction failed: ${err instanceof Error ? err.message : err}`)
    return null
  }
}
...
async function extractHtml(
  ...
  try {
    ...
    console.log(`   [adaptive] HTML extraction failed: ${err instanceof Error ? err.message : err}`)
    return null
  }
}
...
async function extractApi(
  ...
  try {
    const url = resolveUrlTemplate(apiConfig.endpoint, { ...vars, meetingId: meeting.id })
    ...
    console.log(`   [adaptive] API extraction failed: ${err instanceof Error ? err.message : err}`)
    return null
  }
}
...
export async function loadAdaptiveSourcesFromDb(): Promise<MinuteSource[]> {
  ...
  return configs.map(c => ({
    ...
    pdfBasePath: (c.fetcherOptions as Record<string, string> | null)?.pdfBasePath,
  }))
}
