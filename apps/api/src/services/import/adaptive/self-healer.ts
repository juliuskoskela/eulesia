...
export async function runSelfHealing(): Promise<HealResult[]> {
  ...
  console.log(`   [self-healer] Attempting to heal ${failing.length} failing configs...`)
  ...
  for (const config of failing) {
    try {
      ...
      results.push(result)
      console.log(`   [self-healer] ${config.municipalityName}: ${result.action} — ${result.reason}`)
    } catch (err) {
      ...
      console.log(`   [self-healer] ${config.municipalityName}: error — ${msg}`)
    }
  }
 ...
 */
async function healConfig(config: typeof scraperConfigs.$inferSelect): Promise<HealResult> {
  const { id, municipalityName, baseUrl, country, systemType, configVersion } = config
  ...
  if (!analysis || analysis.status === 0) {
    ...
    return { configId: id, municipality: municipalityName, action: 'disabled', reason: 'Page unreachable' }
  }

  if (analysis.status === 404 || analysis.status === 410) {
    await disableConfig(id, `HTTP ${analysis.status}: page removed`)
    return { configId: id, municipality: municipalityName, action: 'disabled', reason: `HTTP ${analysis.status}` }
  }

  if (analysis.status >= 500) {
    // Server error — don't disable yet, might be temporary
    return { configId: id, municipality: municipalityName, action: 'failed', reason: `Server error HTTP ${analysis.status}` }
  }
  ...
  if (analysis.structure?.patterns.detectedSystem) {
    ...
    if (template) {
      ...
      console.log(`   [self-healer] Detected system: ${detected}, using template`)
    }
  }
  ...
  if (!newConfig) {
    ...
    try {
      newConfig = await regenerateConfigWithAi(baseUrl, country, config.config as FetcherConfig, analysis)
      generatedBy = 'ai-self-heal'
    } catch (err) {
      const msg = err instanceof Error ? err.message : String(err)
      return { configId: id, municipality: municipalityName, action: 'failed', reason: `AI generation failed: ${msg}` }
    }
  }
  ...
  console.log(`   [self-healer] Healed ${municipalityName}: v${configVersion} → v${newVersion} (${testResult.meetingCount} meetings found)`)
...
Sample links:
${analysis.structure.sampleLinks.slice(0, 10).map(l => `  - "${l.text}" → ${l.href}`).join('\n')}

PDF links:
${analysis.structure.pdfLinks.slice(0, 5).map(l => `  - ${l}`).join('\n')}
` : ''
...
Generate an updated FetcherConfig JSON that works with the current page structure. Return ONLY the JSON object.`
  ...
  const response = await callAi('self-healing', [
  ...
  if (!validation.success) {
    throw new Error(`AI-generated config validation failed: ${validation.errors.join(', ')}`)
  }
