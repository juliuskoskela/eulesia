...
const rateLimits: Record<string, { lastCallTime: number; delayMs: number }> = {
  mistral: {
    lastCallTime: 0,
    delayMs: parseInt(process.env.MISTRAL_RATE_LIMIT_MS || "500", 10),
  },
    ...
    try {
      ...
      if (attempt < MAX_RETRIES) {
        ...
        if (attempt === MAX_RETRIES - 1 && provider !== "mistral") {
          console.log(
            `   [ai] ${provider} failed, falling back to Mistral: ${errMsg}`,
          );
          try {
            return await callMistralDirect(messages, options);
          } catch {
        ...
        console.log(
          `   [ai] Retry ${attempt + 1}/${MAX_RETRIES} in ${Math.ceil(backoffMs / 1000)}s: ${errMsg}`,
        );
      ...
      throw new Error(
        `AI call failed after ${MAX_RETRIES} retries (${provider}): ${errMsg}`,
      );
    }
  }
