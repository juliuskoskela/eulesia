...
export async function generateFetcherConfig(
  ...
  try {
    const config = await generateConfigWithAi(baseUrl, country, analysis.cleanedHtml, analysis.structure)
    return {
      success: true,
      config,
...
Sample links:
${structure.sampleLinks.slice(0, 10).map(l => `  - "${l.text}" → ${l.href}`).join('\n')}

PDF links:
${structure.pdfLinks.slice(0, 5).map(l => `  - ${l}`).join('\n')}
` : ''
...
Generate a JSON FetcherConfig that can extract meeting listings and content from this page. Return ONLY the JSON object, no explanation.`
  ...
  const response = await callAi('config-generation', [
...
export async function testConfig(
  ...
  try {
    ...
    if (!response.ok) {
      return { success: false, meetingCount: 0, error: `HTTP ${response.status}` }
    }
    ...
    return {
      success: count > 0,
      meetingCount: count,
      error: count === 0 ? 'No meetings found with the configured pattern' : undefined,
    }
  } catch (err) {
    return {
      success: false,
