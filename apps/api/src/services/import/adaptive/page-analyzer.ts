...
export async function analyzePage(url: string): Promise<PageAnalysis | null> {
  try {
    ...
    if (!response.ok) {
      return { url, status: response.status, error: `HTTP ${response.status}`, cleanedHtml: '', structure: null }
    }
...
function extractStructure(html: string): PageStructure {
  ...
  const detectedDateFormat = Object.entries(datePatterns)
    .find(([, found]) => found)?.[0] || null

  // Meeting-related keywords (multi-language)
  const meetingKeywords = [
    'kokous', 'pöytäkirja', 'esityslista',   // Finnish
    'meeting', 'minutes', 'agenda',             // English
    'protokoll', 'sitzung', 'niederschrift',    // German
    'sammanträde', 'kallelse',                  // Swedish
    'délibération', 'procès-verbal', 'séance',  // French
    'istung', 'otsus',                          // Estonian
    'vergadering', 'notulen',                   // Dutch
    'referat', 'møte',                          // Norwegian/Danish
  ]

  const hasMeetingKeywords = meetingKeywords.some(
    kw => html.toLowerCase().includes(kw)
  )

  const protocolKeywords = [
    'pöytäkirja', 'protokoll', 'protocol', 'procès-verbal',
    'minutes', 'niederschrift', 'notulen', 'referat',
  ]

  const hasProtocolKeywords = protocolKeywords.some(
    kw => html.toLowerCase().includes(kw)
  )
  ...
  else if (lowerHtml.includes('ktwebbin') || lowerHtml.includes('ktproxy2')) detectedSystem = 'tweb'
  else if (lowerHtml.includes('allris')) detectedSystem = 'allris'
  else if (lowerHtml.includes('sessionnet')) detectedSystem = 'sessionnet'
  else if (lowerHtml.includes('sdnet') || lowerHtml.includes('sdnetrim')) detectedSystem = 'sdnet'
  else if (lowerHtml.includes('amphora') || lowerHtml.includes('volis')) detectedSystem = 'volis'
