...
import {
  ...
  type CountryConfig,
  type UrlPattern,
} from "./discovery/registry-sources.js";
...
import {
  ...
  buildFiUrl,
  HELSINKI_BODIES,
} from "./discovery/fi-system-map.js";
...
async function seedCountryMunicipalities(
  ...
  for (let i = 0; i < municipalities.length; i++) {
    ...
    if (options.skipExisting) {
      ...
      if (exists) {
        if (options.verbose)
          console.log(`   ${progress} ${muni.name}: already exists, skipping`);
        results.push({
          entity: muni.name,
          country: config.code,
    ...
    if (config.code === "FI") {
      ...
      if (fiInfo) {
        ...
        if (fiInfo.system === "none") {
          console.log(
            `   ${progress} ${muni.name}: ⊘ no meeting system (${fiInfo.notes || "confirmed"})`,
          );
          results.push({
            entity: muni.name,
            country: config.code,
        ...
        if (fiInfo.system === "helsinki-paatokset") {
          ...
          if (!hkiTemplate) {
            console.log(
              `   ${progress} ${muni.name}: ✗ no helsinki-paatokset template`,
            );
            continue;
          }
          ...
          for (const body of HELSINKI_BODIES) {
            ...
            if (options.skipExisting) {
              ...
              if (exists) {
                if (options.verbose)
                  console.log(`     ${body.name}: already exists, skipping`);
                results.push({
                  entity: body.name,
                  country: config.code,
                  adminLevel: "municipality",
        ...
        if (options.verbose)
          console.log(`     Pre-mapped: ${fiInfo.system}, verifying ${url}...`);
        const ok = await probeUrl(url, undefined);
        if (ok) {
          match = { system: fiInfo.system, url };
        } else {
          console.log(
            `   ${progress} ${muni.name}: ⚠ mapped URL failed (${fiInfo.system}), probing all patterns...`,
          );
    ...
    if (!template) {
      console.log(
        `   ${progress} ${muni.name}: ✗ no template for ${match.system}`,
      );
      results.push({
        entity: muni.name,
        country: config.code,
    ...
    if (options.testConfigs) {
      ...
      if (!testResult.success) {
        console.log(
          `   ${progress} ${muni.name}: ⚠ ${match.system} found but test failed (${testResult.error})`,
        );
    ...
    try {
      ...
      const testInfo =
        meetingCount !== undefined ? ` (${meetingCount} meetings)` : "";
  ...
  for (let i = 0; i < entities.length; i++) {
    ...
    if (options.skipExisting) {
      ...
      if (exists) {
        if (options.verbose)
          console.log(
            `   ${progress} ${entity.name}: already exists, skipping`,
          );
        results.push({
          entity: entity.name,
          country: config.code,
    ...
    if (!patterns || patterns.length === 0) {
      console.log(
        `   ${progress} ${entity.name}: ✗ no URL patterns for ${entity.adminLevel}`,
      );
      results.push({
        entity: entity.name,
        country: config.code,
    ...
    if (!template) {
      console.log(
        `   ${progress} ${entity.name}: ✗ no template for ${match.system}`,
      );
      results.push({
        entity: entity.name,
        country: config.code,
    ...
    try {
      ...
      console.log(
        `   ${progress} ${entity.name}: ✓ ${match.system} (${entity.adminLevel})`,
      );
  ...
  for (let i = 0; i < municipalities.length; i++) {
    ...
    try {
      ...
      if (!foundUrl) {
        ...
        console.log(
          `   ${progress} ${muni.name}: ✗ no Amphora/Delta URL found`,
        );
        results.push({
          entity: muni.name,
          country: "EE",
          adminLevel: "municipality",
          systemType: "volis",
          url: `https://atp.amphora.ee/${muni.slug}/`,
          status: "no-match",
        });
        continue;
      }
...
export async function seedConfigs(options: SeedOptions = {}): Promise<{
  ...
  const priorityCountries = options.countries || [
    "FI",
    "SE",
    "NO",
    "DK",
...
async function main() {
  ...
  if (countryIdx !== -1 && args[countryIdx + 1]) {
    options.countries = args[countryIdx + 1]
      .split(",")
  ...
  try {
    ...
    if (failed.length > 0) {
      ...
      for (const f of failed.slice(0, 20)) {
        console.log(
          `   ${f.country} | ${f.entity} | ${f.status} | ${f.error || "no URL found"}`,
        );
      }
      if (failed.length > 20) {
