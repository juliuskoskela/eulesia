...
const KNOWN_SYSTEMS = [
  'cloudnc', 'dynasty', 'tweb',           // Finland
  'allris', 'sessionnet', 'sdnet',         // Germany
  'volis', 'delta',                        // Estonia
  'flexite', 'sweden-generic',             // Sweden
  'webdelib', 'idelbes',                   // France
  'ibabs', 'notubiz',                      // Netherlands
  'norway-generic', 'denmark-generic',     // Nordics
  'spain-generic', 'italy-generic',        // Southern Europe
  'portugal-generic', 'poland-bip',        // Other
  'czech-generic', 'austria-generic',      // Central Europe
]
 ...
 */
export async function classifySystem(url: string): Promise<ClassificationResult> {
...
Sample links:
${analysis.structure.sampleLinks.slice(0, 15).map(l => `  "${l.text}" → ${l.href}`).join('\n')}
...
What CMS/system does this municipality use? Return ONLY the JSON object.`
  ...
  try {
    const response = await callAi('classification', [
    ...
    const result = parseAiJson<{ systemType: string; confidence: string; reasoning: string }>(response)

    const systemType = KNOWN_SYSTEMS.includes(result.systemType) ? result.systemType : 'unknown'
...
export async function classifyAndConfigure(configId: string): Promise<{
  ...
  if (!config) {
    return { classified: false, systemType: 'unknown', configured: false, tested: false, error: 'Config not found' }
  }

  // Step 1: Classify
  console.log(`   [classifier] Classifying ${config.municipalityName} (${config.baseUrl})...`)
  const classification = await classifySystem(config.baseUrl)

  console.log(`   [classifier] Result: ${classification.systemType} (${classification.confidence}) — ${classification.reasoning}`)
  ...
  if (classification.hasTemplate) {
    ...
    if (test.success) {
      ...
      console.log(`   [classifier] Configured with template: ${classification.systemType} (${test.meetingCount} meetings)`)
      return { classified: true, systemType: classification.systemType, configured: true, tested: true }
    }
  ...
  try {
    const result = await generateFetcherConfig(config.baseUrl, config.country, classification.systemType !== 'unknown' ? classification.systemType : undefined)
    ...
    if (result.success && result.config) {
      ...
      if (test.success) {
        ...
        console.log(`   [classifier] Configured with AI (${test.meetingCount} meetings)`)
        return { classified: true, systemType: classification.systemType, configured: true, tested: true }
      }
 ...
 */
export async function processPendingConfigs(options?: { limit?: number }): Promise<{
  processed: number
  activated: number
  errors: string[]
}> {
  ...
  console.log(`   [classifier] Processing ${pending.length} pending configs...`)
  ...
  for (const config of pending) {
    try {
      ...
      errors.push(`${config.municipalityName}: ${err instanceof Error ? err.message : String(err)}`)
    }
  }

  console.log(`   [classifier] Done: ${activated}/${pending.length} activated, ${errors.length} errors`)
