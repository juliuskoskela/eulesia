...
export async function runDiscovery(options: DiscoveryOptions = {}): Promise<{
  ...
  const results: Record<string, { probed: number; found: number; saved: number }> = {}
  ...
  for (const code of countries) {
    ...
    if (!allowedModes.includes(mode as 'production' | 'test' | 'discovery-only')) {
    ...
    try {
      ...
      const useFullDiscovery = options.includeAllLevels || (options.adminLevels && options.adminLevels.length > 0)

      const result = useFullDiscovery
        ? await discoverCountryFull(config, {
            limit: options.limit,
            dryRun: options.dryRun,
            adminLevels: options.includeAllLevels ? undefined : options.adminLevels,
          })
