...
export type OperatingMode = 'production' | 'test' | 'discovery-only' | 'disabled'
...
export function getCountryMode(countryCode: string): OperatingMode {
  ...
  if (envVal && ['production', 'test', 'discovery-only', 'disabled'].includes(envVal)) {
    return envVal
  }
...
const FI_URL_PATTERNS: UrlPattern[] = [
  ...
  {
    system: 'dynasty',
    buildUrl: (slug) => `https://poytakirjat.${slug}.fi/cgi/DREQUEST.PHP?page=meeting_frames`,
    confirmPattern: 'DREQUEST',
  },
  // Dynasty: dynasty.[kunta].fi/djulkaisu
  {
    system: 'dynasty',
    buildUrl: (slug) => `https://dynasty.${slug}.fi/djulkaisu/cgi/DREQUEST.PHP?page=meeting_frames`,
    confirmPattern: 'DREQUEST',
  },
  // Dynasty: www.[kunta].fi/djulkaisu
  {
    system: 'dynasty',
    buildUrl: (slug) => `https://www.${slug}.fi/djulkaisu/cgi/DREQUEST.PHP?page=meeting_frames`,
    confirmPattern: 'DREQUEST',
  },
  // Tweb
  {
  ...
  {
    system: 'dynasty',
    buildUrl: (slug) => `https://${slug}.oncloudos.com/cgi/DREQUEST.PHP?page=meeting_frames`,
    confirmPattern: 'DREQUEST',
  },
  // Dynasty: OnCloudOS with d10 suffix (common variant)
  {
    system: 'dynasty',
    buildUrl: (slug) => `https://${slug}d10.oncloudos.com/cgi/DREQUEST.PHP?page=meeting_frames`,
    confirmPattern: 'DREQUEST',
  },
...
const DK_URL_PATTERNS: UrlPattern[] = [
  {
    ...
    buildUrl: (slug) => `https://www.${slug}.dk/politik/dagsordener-og-referater`,
...
export const COUNTRY_CONFIGS: Record<string, CountryConfig> = {
  FI: {
    ...
    adminUrlPatterns: {
      region: [
        ...
        {
          system: 'dynasty',
          buildUrl: (slug) => `https://${slug}.fi/paatoksenteko/esityslistat-ja-poytakirjat`,
  ...
  SE: {
    ...
    adminUrlPatterns: {
      region: [
        ...
        {
          system: 'sweden-generic',
          buildUrl: (slug) => `https://www.${slug}.se/politik-och-demokrati/protokoll`,
          confirmPattern: 'protokoll',
        },
        {
          system: 'sweden-generic',
          buildUrl: (slug) => `https://www.${slug}.se/om-regionen/politik/sammantraden`,
          confirmPattern: 'protokoll',
        },
      ],
    },
  ...
  AT: {
    ...
    urlPatterns: [{ system: 'austria-generic', buildUrl: (slug) => `https://www.${slug}.gv.at/politik/gemeinderatsprotokolle`, confirmPattern: 'protokoll' }],
  ...
  PL: {
    ...
    language: 'pl',
    defaultMode: 'disabled',
    urlPatterns: [{ system: 'poland-bip', buildUrl: (slug) => `https://bip.${slug}.pl/`, confirmPattern: 'bip' }],
  ...
  CZ: {
    ...
    language: 'cs',
    defaultMode: 'disabled',
    urlPatterns: [{ system: 'czech-generic', buildUrl: (slug) => `https://www.${slug}.cz/samosprava/zastupitelstvo/zapisy-z-jednani`, confirmPattern: 'zápis' }],
  ...
  ES: {
    ...
    language: 'es',
    defaultMode: 'disabled',
    urlPatterns: [{ system: 'spain-generic', buildUrl: (slug) => `https://www.${slug}.es/plenos`, confirmPattern: 'pleno' }],
  ...
  IT: {
    ...
    language: 'it',
    defaultMode: 'disabled',
    urlPatterns: [{ system: 'italy-generic', buildUrl: (slug) => `https://www.comune.${slug}.it/delibere`, confirmPattern: 'deliber' }],
  ...
  PT: {
    ...
    language: 'pt',
    defaultMode: 'disabled',
    urlPatterns: [{ system: 'portugal-generic', buildUrl: (slug) => `https://www.cm-${slug}.pt/atas`, confirmPattern: 'ata' }],
