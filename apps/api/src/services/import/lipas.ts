...
async function fetchLipasPlaces(options: {
  ...
  while (hasMore && page <= maxPages) {
    ...
    if (options.typeCodes?.length) {
      options.typeCodes.forEach((code) =>
        params.append("type-codes", code.toString()),
      );
    }

    if (options.municipalityCodes?.length) {
      options.municipalityCodes.forEach((code) =>
        params.append("city-codes", code.toString()),
      );
    }
    ...
    try {
      ...
      if (!response.ok) {
        consecutiveErrors++;
        console.log(
          `   ⚠️  Page ${page} failed: ${response.status} (attempt ${consecutiveErrors}/${maxConsecutiveErrors})`,
        );
      ...
      consecutiveErrors++;
      console.log(
        `   ⚠️  Page ${page} error: ${err} (attempt ${consecutiveErrors}/${maxConsecutiveErrors})`,
      );
...
function parseLipasPlace(place: LipasPlace): {
  ...
  const mapping = typeCode
    ? TYPE_MAPPING[typeCode] || DEFAULT_MAPPING
    : DEFAULT_MAPPING;
 ...
 */
export async function importFromLipas(
  options: LipasImportOptions = {},
  ...
  console.log(
    `   Municipality codes: ${municipalityCodes?.join(", ") || "all"}`,
  );
  ...
  try {
    ...
    const lipasPlaces = await fetchLipasPlaces({
      typeCodes,
      municipalityCodes,
      limit,
    });
    ...
    if (lipasPlaces.length > 0) {
      console.log(
        `   Sample item:`,
        JSON.stringify(lipasPlaces[0], null, 2).slice(0, 500),
      );
    }
    ...
    for (let i = 0; i < parsedPlaces.length; i += batchSize) {
      ...
      console.log(
        `   Processed ${Math.min(i + batchSize, parsedPlaces.length)}/${parsedPlaces.length}`,
      );
    }
  } catch (err) {
