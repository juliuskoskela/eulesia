 ...
 */
export async function parseFeed(feedUrl: string, limit = 20): Promise<FeedResult> {
...
function parseRss(xml: string, limit: number): FeedResult {
  const feedTitle = extractTag(xml, 'channel>title') || extractTag(xml, 'title') || 'Unknown Feed'
...
function parseAtom(xml: string, limit: number): FeedResult {
  ...
  while ((match = entryRegex.exec(xml)) !== null && items.length < limit) {
    ...
    const summary = decodeXmlEntities(stripHtml(extractTag(entryXml, 'summary') || extractTag(entryXml, 'content') || ''))
