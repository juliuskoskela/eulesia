...
export const MINISTRY_SOURCES: MinistrySource[] = [
  {
    name: "Valtioneuvosto",
    feedUrl:
      "https://valtioneuvosto.fi/en/staattiset-feedit-en/-/asset_publisher/LOmkEPY4nk2s/rss",
    contentType: "press",
    language: "en",
  },
  {
    ...
    for (const decLink of session.decisions) {
      ...
      try {
        ...
        if (threadScope === "local" && summary.region) {
          ...
          const normalized =
            summary.region.charAt(0).toUpperCase() +
            summary.region.slice(1).toLowerCase();
          const [muni] = await db
        ...
        const [thread] = await db
          ...
          .values({
            ...
            institutionalContext: {
              ...
              publishedAt:
                parseFinDate(decision.sessionDate)?.toISOString() ||
      ...
      for (const item of recentItems) {
        ...
        try {
          ...
          if (rssScope === "local" && summary.region) {
            rssLocationId = await resolveLocationForMunicipality(
              summary.region,
            );
            const normalized =
              summary.region.charAt(0).toUpperCase() +
...
export async function importMinistryContent(
  ...
  if (!skipRss) {
    try {
      await importRssFeeds(botUserId, result, {
        dryRun,
        limit: 10,
        filterSources,
      });
 ...
 */
export async function cleanOldMinistryThreads(
  options: { dryRun?: boolean; cutoffDate?: Date } = {},
): Promise<number> {
  const { dryRun = false, cutoffDate = new Date("2025-01-01") } = options;

  console.log(
    `🧹 Cleaning ministry threads published before ${cutoffDate.toISOString().slice(0, 10)}...`,
  );
  console.log(`   Dry run: ${dryRun}`);
  ...
  const oldThreads = await db
    .select({
      id: threads.id,
      title: threads.title,
      sourceId: threads.sourceId,
      context: threads.institutionalContext,
    })
    .from(threads)
  ...
  console.log(
    `   Found ${toDelete.length} threads to remove (of ${oldThreads.length} total rss_import threads)`,
  );
  ...
  if (dryRun) {
    for (const t of toDelete) {
      const ctx = t.context as { publishedAt?: string } | null;
      console.log(
        `   🗑️  Would delete: ${t.title?.slice(0, 60)} (${ctx?.publishedAt?.slice(0, 10)})`,
      );
    }
  ...
  for (const t of toDelete) {
    ...
    const ctx = t.context as { publishedAt?: string } | null;
    console.log(
      `   🗑️  Deleted: ${t.title?.slice(0, 60)} (${ctx?.publishedAt?.slice(0, 10)})`,
    );
  }
