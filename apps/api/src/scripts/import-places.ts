...
import { importFromOSM, getAvailableCategories, getSupportedCountries } from '../services/import/osm.js'
import { importFromLipas, getAvailableTypeCodes } from '../services/import/lipas.js'
...
LIPAS TYPE CODES (Finland only):
  ${getAvailableTypeCodes().slice(0, 5).map((t: { code: number; name: string }) => `${t.code}: ${t.name}`).join(', ')}...
